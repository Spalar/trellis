...
class ImpactAnalyzer:
    ...
    def analyze_impact(self, symbol: str, depth: int = 2) -> Dict:
        ...
        risk_level = self._calculate_risk(
            len(caller_functions), len(caller_files), feature_impacts
        )
    ...
    def _analyze_feature_impacts(
        self, symbol: str, affected_functions: List[Dict]
    ) -> List[FeatureImpact]:
    ...
    def _analyze_feature_risks(self, impact: FeatureImpact) -> None:
        ...
        if feature.status == "deprecated":
            impact.risk_flags.append(
                "Feature is deprecated - changes may indicate migration needed"
            )
    ...
    def _calculate_risk(
        self, affected_count: int, file_count: int, feature_impacts: List[FeatureImpact]
    ) -> str:
        ...
        cross_feature = len(
            [fi for fi in feature_impacts if fi.feature_name != "unassigned"]
        )
    ...
    def _get_development_pointers(
        self, symbol: str, feature_impacts: List[FeatureImpact]
    ) -> List[str]:
        ...
        if feature:
            ...
            for decision in feature.decisions:
                pointers.append(
                    f"Decision {decision.decision_id}: {decision.description}"
                )
            ...
            if feature.status == "deprecated":
                pointers.append(
                    "WARNING: Feature is deprecated - changes should be for migration only"
                )
        ...
        other_features = [
            ...
            for fi in feature_impacts
            if fi.feature_name
            not in (feature.feature_name if feature else "", "unassigned")
        ]
        if other_features:
            pointers.append(
            ...
            for fi in other_features:
                pointers.append(
                    f"  - {fi.feature_name}: {len(fi.impacted_functions)} functions"
                )
