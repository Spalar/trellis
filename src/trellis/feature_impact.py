...
class FeatureImpactAnalyzer:
    ...
    def get_development_pointers(self, symbol: str) -> List[str]:
        ...
        pointers.append(
            f"Feature: {context['feature_name']} - {context['description']}"
        )
        ...
        if context["status"] == "deprecated":
            ...
            pointers.append(
                "INFO: Feature is planned - ensure changes align with roadmap"
            )
    ...
    def _generate_summary(
        self, symbol: str, feature_impacts: List[FeatureImpact], divergence: List[str]
    ) -> str:
        ...
        lines.append(
            f"Affects {total_funcs} functions across {total_features} features"
        )
